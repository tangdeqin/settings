...
public class ApnEditor extends SettingsPreferenceFragment
    ...
    //Begin added by keyong.yu for APN REQ on 2018/07/18
    private boolean isMvnoTypeNone = false;
    private static final String SOURCE_TYPE = "sourcetype";
    private int mSourceType = SOURCE_TYPE_DEFAULT;
    //for [Read Only APN]
    ...
    public static final int SOURCE_TYPE_USER_ADDED = 1; // from user added
    private static final int SOURCE_TYPE_INDEX = 25;
    private static final String DIALOG_FRAGMENT_TAG =
            "android.support.v14.preference.PreferenceFragment.DIALOG";
    //End added by keyong.yu for APN REQ on 2018/07/18
    
    //Begin added by shengjiao.liu.hz for XR7069617 on 2018/10/30
    private boolean mIsApnListhaschanged = false;
    //End added by shengjiao.liu.hz for XR7069617 on 2018/10/30
    /**
    ...
    private static final String[] sProjection = new String[] {
            ...
            //Begin added by shengjiao.liu.hz for APN REQ on 2018/09/04
            SOURCE_TYPE //25
            //End added by shengjiao.liu.hz for APN REQ on 2018/09/04
    ...
    public void onCreate(Bundle icicle) {
        ...
        // Begin deleted by kun.mu for XR5945452 on 2018/01/31
        // Use TCL customization about edit or not
        ...
        // End deleted by kun.mu for XR5945452 on 2018/01/31
        //Begin added by keyong.yu for APN REQ on 2018/07/18
        boolean isDisableRoamingField = getResources().getBoolean(R.bool.def_disable_apn_roaming_protocol_field);
        if (isDisableRoamingField) {
    ...
    public void onResume() {
        ...
        //Begin added by shengjiao.liu.hz for APN REQ on 2018/09/04
        //for [Read Only APN]
        //getPreferenceScreen().setEnabled(!mReadOnlyMode);//Deleted by miaoliu for XR7226032 on 2018/12/19
        //End added by shengjiao.liu.hz for APN REQ on 2018/09/04
    ...
    private void fillUi() {
        if (mFirstTime) {
            ...
            if (mNewApn) {
                mSourceType = SOURCE_TYPE_USER_ADDED;
                int protocolIndex = getResources().getInteger(R.integer.def_default_protocol_new_APN);
                mProtocol.setValueIndex(protocolIndex);
                mRoamingProtocol.setValueIndex(protocolIndex);
                
                mApnType.setText("default");
                int defaultPort = getResources().getInteger(R.integer.def_settings_defaultPortInApn);
                if (defaultPort > 0 && defaultPort <= 65535) {
                    if ("".equals(checkNotSet(mProxy.getText()))
                            && "".equals(checkNotSet(mPort.getText()))) {
                        mPort.setText(Integer.toString(defaultPort));
                    }
                }
            }
            boolean isDisableBearer = getResources().getBoolean(R.bool.def_settings_apn_disable_bearer_item);
            if (isDisableBearer) {
                mBearerMulti.setEnabled(false);
            }
        ...
        //Begin added by shengjiao.liu.hz for APN REQ on 2018/09/04
        String mccmncs = getResources().getString(R.string.def_settings_set_none_mvno_type_list);
        if (mccmncs != null && mccmncs.equalsIgnoreCase("all")) {
            isMvnoTypeNone = true;
        }
        if (!isMvnoTypeNone) {
            String[] mccmncList = mccmncs.split(",");
            //Begin modified by shengjiao.liu.hz for XR7119298 on 2018/11/16
            //String plmn = mCursor.getString(MCC_INDEX) + mCursor.getString(MNC_INDEX);
            String plmn = mTelephonyManager.getSimOperator(mSubId);
            //End modified by shengjiao.liu.hz for XR7119298 on 2018/11/16
            if (mccmncList != null && plmn != null && !("".equals(plmn))) {
                for (String mccmnc : mccmncList) {
                    Log.d(TAG, "mccmncList() mccmnc=" + mccmnc);
                    if (mccmnc != null && mccmnc.equalsIgnoreCase(plmn)) {
                        isMvnoTypeNone = true;
                        break;
                    }
                }
        ...
        //Begin added by shengjiao.liu.hz for XR7442061 on 2019/02/15
        if (!isMvnoTypeNone && "23430".equals(mTelephonyManager.getSimOperator(mSubId)) 
                      && "pnn".equalsIgnoreCase(mCursor.getString(MVNO_TYPE_INDEX)) && "EE".equals(mCursor.getString(MVNO_MATCH_DATA_INDEX))) {
            Log.d(TAG, "set isMvnoTypeNone as true for EE Consumer SIM");
            isMvnoTypeNone = true;
        }
        //End added by shengjiao.liu.hz for XR7442061 on 2019/02/15

        TelephonyManager tm = (TelephonyManager) getActivity().getSystemService(Context.TELEPHONY_SERVICE);
        String numeric = tm.getSimOperator(mSubId);
        boolean isOpenMacktSim = TclPluginManager.getTclSdkAdapter(getActivity()).isOpenMarketSim(numeric); //Modify by chenli.gao.hz for XR6998744 on 2018/09/12

        if ((isMvnoTypeNone || isOpenMacktSim) && (mSourceType == SOURCE_TYPE_DEFAULT
                || mSourceType == SOURCE_TYPE_USER_ADDED || mNewApn)) {
            mMvnoType.setEnabled(false);
            mMvnoMatchData.setEnabled(false);

            mMvnoType.setSummary(checkNull(""));
            mMvnoMatchData.setSummary(checkNull(""));
        } else {
        //End added by shengjiao.liu.hz for APN REQ on 2018/09/04
    ...
    //Begin added by keyong.yu for APN REQ on 2018/07/18
    /**
     * override to show EditTextPreference dialog in ApnEditor page specially.
     * @param preference
     */
    @Override
    public void onDisplayPreferenceDialog(Preference preference) {
        ...
        if(preference instanceof EditTextPreference) {
            if(!(preference instanceof CustomEditTextPreference)) {
                Log.d(TAG, "onDisplayPreferenceDialog: handle...");
                if (preference.getKey() == null) {
                    // Auto-key preferences that don't have a key, so the dialog can find them.
                    preference.setKey(UUID.randomUUID().toString());
                }
                DialogFragment f = ApnEditTextPreferenceDialogFragment.newInstance(preference.getKey());
    ...
    /**
     * Subclass to define TextView selection.
     */
    public static class ApnEditTextPreferenceDialogFragment extends EditTextPreferenceDialogFragment {

        private static String preferenceKey = null;

        public static ApnEditTextPreferenceDialogFragment newInstance(String key) {
            Log.d(TAG, "ApnEditTextPreferenceDialogFragment newInstance");
            preferenceKey = key;
            final ApnEditTextPreferenceDialogFragment
                    fragment = new ApnEditTextPreferenceDialogFragment();
        ...
        protected void onBindDialogView(View view) {
            ...
            if(editText != null) {
                Log.d(TAG, "onBindDialogView: editText.getText() = " + editText.getText());
                editText.setSelection(editText.getText().length());
                editText.requestFocus();//Added by miaoliu for XR7342814 on 2019/1/16
                Log.d(TAG, "onBindDialogView: preferenceKey = "+preferenceKey);
                switch (preferenceKey) {
                    case "apn_apn":
                    case "apn_user":
                    case "apn_password":
                        Log.d(TAG, "onBindDialogView: set preference EditText length limit to 32");
                        editText.setFilters(new InputFilter[]{new InputFilter.LengthFilter(32)});
                        break;
                    case "apn_http_port":
                    case "apn_mms_port":
                        Log.d(TAG, "onBindDialogView: set preference EditText length limit to 5");
    ...
    private String mvnoDescription(String newValue) {
        ...
        if (mvnoIndex == -1) {
            ...
            //Begin modified by shengjiao.liu.hz for XR7031603 on 2018/11/23
            //String[] values = mRes.getStringArray(R.array.mvno_type_entries);
            String[] values = mRes.getStringArray(R.array.mvno_type_entries_ext);
            //End modified by shengjiao.liu.hz for XR7031603 on 2018/11/23
            ...
            if (newValue != null && newValue.equals(oldValue) == false) {
                ...
                //Begin added by shengjiao.liu.hz for XR7031603 on 2018/11/23
                    ...
                    String pnn = Settings.Global.getString(getContext().getContentResolver(), "firstPnn");
                    mMvnoMatchData.setText(pnn);
                }
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        //super.onCreateOptionsMenu(menu, inflater);//remove by liu.zheng.hz for task 6190881 at 20180412
        ...
        //End added by shengjiao.liu.hz for APN REQ on 2018/09/04
        boolean isApnEditable = getResources().getBoolean(R.bool.def_apn_edit_exit_flg);
        if ((!mNewApn && !mReadOnlyApn && mSourceType != SOURCE_TYPE_DEFAULT) || isApnEditable) {
        //End modified by keyong.yu for APN REQ on 2018/07/18
            menu.add(0, MENU_DELETE, 0, R.string.menu_delete)
                .setIcon(R.drawable.ic_menu_delete);
        }
    ...
    public boolean onKey(View v, int keyCode, KeyEvent event) {
        ...
        switch (keyCode) {
            case KeyEvent.KEYCODE_BACK: {
                ...
                if (isExitDirectly(mSourceType)) {
                    Log.i(TAG, "onKeyDown: back key press and not save the APN info.");
                    finish();
                }
                else {
                    //Begin modified by shengjiao.liu.hz for XR7069617 on 2018/10/30
                    if(mIsApnListhaschanged && !showSaveApnDialog()) {
                        if (validateAndSave(false)) {
                            finish();
                        }
                    }
                    else if(!mIsApnListhaschanged) {
                        finish();
                    }
                    //End modified by shengjiao.liu.hz for XR7069617 on 2018/10/30
                }
                //End modified by keyong.yu for APN REQ on 2018/07/18
                return true;
            }
    ...
    public void onSaveInstanceState(Bundle icicle) {
        ...
        if ((mSourceType == SOURCE_TYPE_DEFAULT)
                && isMvnoTypeNone) {
            Log.i(TAG, "onKeyDown: back key press and not save the APN info.");
            //finish();
        } else {
    ...
    //Begin added by shengjiao.liu.hz for XR6973017 on 2018/09/21
    private boolean isExitDirectly(int sourceType) {
        boolean isExitDirect = getResources().getBoolean(R.bool.def_exitdirectly_after_backkey);
        if (isExitDirect ||((sourceType == 0) && isMvnoTypeNone)) {
            return true;
        } else
            return false;
    }
    //End added by shengjiao.liu.hz for XR6973017 on 2018/09/21
    
    //Begin added by shengjiao.liu.hz for XR7069617 on 2018/10/30
    public boolean showSaveApnDialog() {
        boolean askUserSave = getResources().getBoolean(R.bool.def_apn_ask_user_save);
        if (!askUserSave) {
            return false;
        } else {
            boolean isDefaultApn = mSourceType == SOURCE_TYPE_DEFAULT;
            android.app.AlertDialog dialog = new android.app.AlertDialog.Builder(getContext())
                    .setTitle(android.R.string.dialog_alert_title)
                    .setIcon(android.R.drawable.ic_dialog_alert)
                    .setMessage(isDefaultApn ? getContext().getResources().getString(R.string.apn_predefine_change_dialog_notice)
                            : getContext().getResources().getString(R.string.apn_editor_save))
                    .setCancelable(true)
    ...
    private void updateScreenEnableState() {
        ...
        getPreferenceScreen().setEnabled(!airplaneModeEnabled && !mReadOnlyMode);
    }
