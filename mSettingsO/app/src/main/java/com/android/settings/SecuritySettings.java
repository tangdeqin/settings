...
import com.android.settings.privatemode.SetupPrivacyModeIntroduction;
import com.android.settings.privatemode.SetupScreenLockFinish;
import com.android.settings.applock.AppsLockActivity;
...
public class SecuritySettings extends SettingsPreferenceFragment
    ...
    private boolean mIsPrivateModeOn = false;
    private static final String KEY_APPS_LOCK = "apps_lock";
    //End added by dongchi.chen for XR6700377 on 2018/08/02
    ...
    private LockScreenNotificationPreferenceController mLockScreenNotificationPreferenceController;
     private static final String SECURITY_CATEGORY_KEY_PREFIX = "security_category_";//Added by miaoliu for XR6628992 on 2018/7/24
    @Override
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mIsPrivateModeOn = mTctPrivacyModeHelper.isPrivateModeFeatureOn();
        //End added by dongchi.chen for XR6700377 on 2018/08/02
    }
    ...
    private PreferenceScreen createPreferenceHierarchy() {
        ...
        //Begin modified by zengjie for XR5953635 on 1/31/18
        //addPreferencesFromResource(resid);
        if(resid != 0){
            addPreferencesFromResource(resid);
        }
        //End modified by zengjie for XR5953635 on 1/31/18
        ...
        if (securityCategory != null) {
            ...
            //maybeAddFingerprintPreference(root, mProfileChallengeUserId);
            maybeAddFingerprintSensorPreference(securityCategory, UserHandle.myUserId());
            //End modified by jinlong.lu for XR6618444 on 18-7-31
            //Begin added by dongchi.chen for XR6700377 on 2018/08/02
            maybeAddAppsLockNPrivateModePreference(securityCategory);
            //End added by dongchi.chen for XR6700377 on 2018/08/02
            numberOfTrustAgent = addTrustAgentSettings(securityCategory);
            setLockscreenPreferencesSummary(securityCategory);
            //Begin added by dongchi.chen for XR6700377 on 2018/08/02
            final Preference lockscreenPreferences = ((Preference)findPreference(KEY_LOCKSCREEN_PREFERENCES));
            if (lockscreenPreferences != null) {
                lockscreenPreferences.setOrder(8);//Modified by miaoliu for XRP23119 on 2018/8/30
            }
            //End added by dongchi.chen for XR6700377 on 2018/08/02
        }
        ...
        if (tilePrefs != null && !tilePrefs.isEmpty()) {
            for (Preference preference : tilePrefs) {
                ...
                //Begin added by miaoliu for XR6628992 on 2018/7/24
                } else if(!TextUtils.isEmpty(preference.getKey())
                    && preference.getKey().startsWith(SECURITY_CATEGORY_KEY_PREFIX) && securityCategory != null){
                    securityCategory.addPreference(preference);
                //End added by miaoliu for XR6628992 on 2018/7/24
                }else{
    ...
    //Begin added by jinlong.lu for XR6618444 on 18-8-1
    private void maybeAddFingerprintSensorPreference(PreferenceGroup securityCategory, int userId) {
        Preference fingerprintPreference = FingerprintSensorSettings.FpSettingsFragment.getFingerprintPreferenceForUser(
                securityCategory.getContext(), getChildFragmentManager(), userId);
        if (fingerprintPreference != null) {
            fingerprintPreference.setOnPreferenceChangeListener(this);
            fingerprintPreference.setOrder(1);
    ...
    //Begin added by dongchi.chen for XR6700377 on 2018/08/02
    private void maybeAddAppsLockNPrivateModePreference(PreferenceGroup securityCategory ) {
        //Begin modified by dongchi.chen for XRP10025306 on 20101102
        if(!android.os.UserManager.isDeviceInDemoMode(getContext())) {
            Preference appslock = new Preference(getActivity());
            appslock.setTitle(R.string.apps_lock);
            appslock.setPersistent(false);
            appslock.setKey(KEY_APPS_LOCK);
            appslock.setOrder(5);//Modified by miaoliu for XRP23119 on 2018/8/30
            securityCategory.addPreference(appslock);
            if (TctAppLockHelper.createHelper(getContext()).isAppsLockEnable()) {
                appslock.setSummary(R.string.apps_lock_on);
            } else {
                appslock.setSummary(R.string.apps_lock_off);
            }
        }
        //End modified by dongchi.chen for XRP10025306 on 20101102
        if (mIsPrivateModeOn) {
            Preference privacyMode = new Preference(getActivity());
            privacyMode.setTitle(R.string.private_mode);
            privacyMode.setPersistent(false);
            privacyMode.setKey(KEY_PRIVACY_MODE);
            privacyMode.setOrder(6);//Modified by miaoliu for XRP23119 on 2018/8/30
            securityCategory.addPreference(privacyMode);
            if (mTctPrivacyModeHelper.isPrivacyModeEnable()) {
                privacyMode.setSummary(R.string.private_mode_on);
            } else {
                privacyMode.setSummary(R.string.private_mode_off);
            }
        }
    }
    //End added by dongchi.chen for XR6700377 on 2018/08/02
    ...
    void setLockscreenPreferencesSummary(PreferenceGroup group) {
        ...
        if (lockscreenPreferences != null) {
            //Begin added by shilei.zhang for XR7139307 on 2018/11/20
            mLockScreenNotificationPreferenceController.UpdatemSecure();
            //End added by shilei.zhang for XR7139307 on 2018/11/20
    ...
    private int addTrustAgentSettings(PreferenceGroup securityCategory) {
        ...
        for (int i = 0; i < agents.size(); i++) {
            ...
            trustAgentPreference.setOrder(10);//Added by miaoliu for XRP23119 on 2018/8/30
    ...
    public void onResume() {
        ...
        //Begin added by shilei.zhang for XR7139307 on 2018/11/20
        if(mLockScreenNotificationPreferenceController != null) {
            mLockScreenNotificationPreferenceController.UpdatemSecure();
        }
        //End added by shilei.zhang for XR7139307 on 2018/11/20
    }
    ...
    public boolean onPreferenceTreeClick(Preference preference) {
        ...
        //Begin added by dongchi.chen for XR6700377 on 2018/08/02
        }else if (KEY_PRIVACY_MODE.equals(key)) {
            SetupPrivacyModeIntroduction.enterPrivateModeSettings(getContext());
        }
        else if (KEY_APPS_LOCK.equals(key)) {
            getContext().startActivity(AppsLockActivity.createIntent(getContext()));
        //End added by dongchi.chen for XR6700377 on 2018/08/02
        //Begin added by jinlong.lu for XR6618444 on 18-8-1
        }else if (FingerprintSensorSettings.FpSettingsFragment.KEY_FINGERPRINT_SETTINGS_FRAGMENT.equals(key)) {
            //fingerprint on click
            Intent modIntent = new Intent(getContext(), FingerprintSensorSettings.class);
    ...
    private static class SecuritySearchIndexProvider extends BaseSearchIndexProvider {
        ...
        public List<SearchIndexableRaw> getRawDataToIndex(Context context, boolean enabled) {
            ...
            if (fpm != null && fpm.isHardwareDetected()) {
                ...
                data.title = res.getString(R.string.fingerprints_sensor_fragment);
                data.screenTitle = screenTitle;
                data.iconResId = R.drawable.ic_settings_fingerprint;
                data.key = FingerprintSensorSettings.FpSettingsFragment.KEY_FINGERPRINT_SETTINGS_FRAGMENT;
                result.add(data);
            ...
            //Begin added by dongchi.chen for XR6700377 on 2018/08/02
            boolean isPrivateModeOn = TctPrivacyModeHelper.createHelper(context).isPrivateModeFeatureOn();
            if (isPrivateModeOn) {
                data = new SearchIndexableRaw(context);
                data.title = res.getString(R.string.private_mode);
                data.screenTitle = screenTitle;
                result.add(data);
            }
            data = new SearchIndexableRaw(context);
            data.title = res.getString(R.string.apps_lock);
            data.screenTitle = screenTitle;
            result.add(data);
            //End added by dongchi.chen for XR6700377 on 2018/08/02
            return result;
    ...
    static class SummaryProvider implements SummaryLoader.SummaryProvider {
        ...
        public void setListening(boolean listening) {
            if (listening) {
                ...
                //Begin modified by miaoliu for XR23119 on 2018/8/22
               String summary="";
                if (fpm != null && fpm.isHardwareDetected()) {
                    summary = mContext.getString(R.string.security_dashboard_summary);                
                } else {
                    summary = mContext.getString(R.string.security_dashboard_summary_no_fingerprint);
                }
                if(Utils.checkPackageExist(mContext,"com.tcl.faceunlock")){
                    String facekeySummary = mContext.getString(R.string.security_dashboard_summary_face_key);
                    summary = mContext.getString(R.string.join_many_items_middle, summary,
                        facekeySummary);
                }
                 mSummaryLoader.setSummary(this,summary);
                 //End modified by miaoliu for XR23119 on 2018/8/22
            }
        }
    }
