...
import com.android.settings.privatemode.PrivacyModeSettings;

import tct.appclone.PackageCloneManager;
//End added by dongchi.chen for XR6167859
import android.content.ComponentName;//Added by miaoliu for XRP23297 on 2018/9/6
import com.android.settings.notification.ZenModeSettings;//Added by nanbing.zou for P10024859 on 2018-10-25

// Begin added by weijun.pan for XR7139237 on 2018/11/20
import com.android.settings.bluetooth.BluetoothPairingDetail;
// End added by weijun.pan for XR7139237 on 2018/11/20
...
public final class Utils extends com.android.settingslib.Utils {
     ...
     */
    public static final int UPDATE_PREFERENCE_FLAG_SET_TITLE_TO_MATCHING_ACTIVITY = 0;//Modified by miaoliu for XRP23119 on 2018/9/1
    ...
    private static SparseArray<Bitmap> sDarkDefaultUserBitmapCache = new SparseArray<Bitmap>();
     ...
     public static final String SMART_MANAGER_PACKAGE_NAME = "com.tct.onetouchbooster";//Added by miaoliu for XRP24381 on 2018/9/28
    ...
    public static void prepareCustomPreferencesList(
        ...
        if (movePadding) {
            ...
            final int paddingSide = res.getDimensionPixelSize(R.dimen.settings_side_margin);
             //Begin modified by miaoliu for XR6873798 on 2018/8/25
            // final int paddingBottom = res.getDimensionPixelSize(
            //         com.android.internal.R.dimen.preference_fragment_padding_bottom);
            final int paddingBottom = res.getDimensionPixelSize(
                    R.dimen.preference_fragment_padding_bottom);
            //End modified by miaoliu for XR6873798 on 2018/8/25
    ...
    public static Intent onBuildStartFragmentIntent(Context context, String fragmentName,
        ...
        //Begin Modified by nanbing.zou for P10024859 on 2018-10-25
        if(isExtSubsettingsFragment(fragmentName)){
            intent.setClass(context, SubSettings.SubSettingsExt.class);
        }else {
            intent.setClass(context, SubSettings.class);
        }
        //End Modified by nanbing.zou for P10024859 on 2018-10-25
    ...
    public static UserHandle getManagedProfile(UserManager userManager) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (userInfo.isManagedProfile()
                    /// Added by yunqi.song.hz for JoyUI dualApp TASK 6766810
                    && userInfo.id != PackageCloneManager.getInstance().getCloneUserId()) {
                return profile;
            }
        }
        return null;
   ...
   private static boolean isProfileOf(UserManager um, UserHandle otherUser) {
    ...
    public static CharSequence formatElapsedTime(Context context, double millis,
        ...
        if (measureArray.length == 1 && MeasureUnit.MINUTE.equals(measureArray[0].getUnit())) {
         ...
         //Begin added by miaoliu for XR7139460 on 2018/11/21
        if(locale.getDisplayLanguage().equals("magyar")){
            String s = sb.toString();
            s = s.replaceFirst("h és","óra és");
            s = s.replaceFirst("min","perc");
            //Log.d("miao","replace s="+s+" original sb="+sb);
            return s;
        }
        //End added by miaoliu for XR7139460 on 2018/11/21
        return sb;
    }
    ...
    //Begin Modified by nanbing.zou for P10024859 on 2018-10-25
    //Begin added by yucheng.luo for XR6151603 on 18-04-02
    private static final String[] ENTRY_FRAGMENTS = {
            //Begin added by dongchi.chen for XR6700377 on 2018/08/02
            PrivacyModeSettings.class.getName(),
    ...
    protected static boolean isExtSubsettingsFragment(String fragmentName) {
        for (int i = 0; i < ENTRY_FRAGMENTS.length; i++) {
            if (ENTRY_FRAGMENTS[i].equals(fragmentName)) return true;
        }
        return false;
    }
    //End added by yucheng.luo for XR6151603 on 18-04-02

    //Begin added by miaoliu for Settings ergo 2.0.2 on 2018/8/22
     public static boolean checkPackageExist(Context context, String packageName) {
        ...
        try {
            ...
            Log.e(TAG, "checkPackageExist exception:" + e.getMessage());
            //End added by junyan.wan.hz for XR 7238303 2019/01/04
            return false;
        }
    }
    //End added by miaoliu for Settings ergo 2.0.2 on 2018/8/22
    //Begin added by miaoliu for XRP23297 on 2018/9/6
    public static boolean checkActivityExist(Context context, ComponentName componetName) {
    ...
    public static String getActivityLabel(Context context, ComponentName componetName) {
        android.content.pm.ActivityInfo info = null;
        try {
            info = context.getPackageManager().getActivityInfo(componetName, 0);
            ApplicationInfo appInfo = context.getPackageManager().getApplicationInfo(componetName.getPackageName(), 0);
            if (info.labelRes != 0) {
                CharSequence label = context.getPackageManager().getText(componetName.getPackageName(), info.labelRes, appInfo);
                if (label != null) {
                    return label.toString().trim();
                }
            }
            return componetName.getPackageName();
        } catch (Exception e) {
            return componetName.getPackageName();
        }
    }
    //End added by miaoliu for XRP23297 on 2018/9/6
   //Begin added by miaoliu for XR7025065 on 2018/10/18
    public static Intent getMobileNetworkSettingsIntent(Context context){
            Intent intent = new Intent(Intent.ACTION_MAIN);
            if (checkPackageExist(context, "com.qualcomm.qti.networksetting")) {
    ...
    //End added by miaoliu for XR7025065 on 2018/10/18
    //Begin modified by miaoliu for XR7139665 on 2018/11/21
    //Begin added by miaoliu for XR7119445 on 2018/11/17
    public static int getIdentifierId(Resources res, String resKey, String resType, String packageName){
        if (res != null && !TextUtils.isEmpty(resKey) && !TextUtils.isEmpty(packageName)) {
              return res.getIdentifier(resKey, resType, packageName);
        }else{
            return 0;
        }
    }
    ...
    public static boolean getBoolean(Context context, String resKey, String packageName, boolean defaultValue){
       Resources res = getPackageResources(context, packageName); 
       int resId = getIdentifierId(res, resKey, "bool", packageName);
       //Log.d("miao","resKey ="+resKey+"   resID = "+resId);
        return resId > 0 ? res.getBoolean(resId) : defaultValue;
    }

    public static boolean getBoolean(Context context, String resKey, String packageName){
        return getBoolean(context, resKey, packageName, false);
    }

    public static int getInteger(Context context, String resKey, String packageName, int defaultValue){
        Resources res = getPackageResources(context, packageName); 
        int resId = getIdentifierId(res, resKey, "integer", packageName);
        return resId > 0 ? res.getInteger(resId) : defaultValue;
    }

    public static int getInteger(Context context, String resKey, String packageName){
       return getInteger(context, resKey, packageName, 0);
    }

    public static String getString(Context context, String resKey, String packageName, String defaultValue){ 
        Resources res = getPackageResources(context, packageName);    
        int resId = getIdentifierId(res, resKey, "string", packageName);
        return resId > 0 ? res.getString(resId) : defaultValue;
    }

    public static String getString(Context context, String resKey, String packageName){
       return getString(context, resKey, packageName, "");
    }
    //End added by miaoliu for XR7119445 on 2018/11/17
    private static Resources getPackageResources(Context context, String packageName){
         if(context != null){
             try {
