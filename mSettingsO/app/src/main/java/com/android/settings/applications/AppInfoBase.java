...
public abstract class AppInfoBase extends SettingsPreferenceFragment
    ...
    protected String retrieveAppEntry() {
        ...
        int extraUserId = getIntent().getIntExtra(Intent.EXTRA_USER_ID, -1);
        mUserId = (extraUserId != -1) ? extraUserId : UserHandle.myUserId();
        ///TCT}
        ...
        if (mAppEntry != null) {
            ...
            try {
                ...
                if(mUserId == PackageCloneManager.getInstance().getCloneUserId()) {
                    if(mUserId != UserHandle.getUserId(mPackageInfo.applicationInfo.uid)) {
