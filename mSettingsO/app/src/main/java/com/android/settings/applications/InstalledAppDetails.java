...
public class InstalledAppDetails extends AppInfoBase
    ...
    public void onCreate(Bundle icicle) {
        ...
        if(mUserId != PackageCloneManager.getInstance().getCloneUserId()) addDynamicPrefs();
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        if(mUserId == PackageCloneManager.getInstance().getCloneUserId()) {
            mNotificationPreference.setVisible(false);
            mStoragePreference.setVisible(false);
            mPermissionsPreference.setVisible(false);
            if (mDataPreference != null) mDataPreference.setVisible(false);
            mBatteryPreference.setVisible(false);
            mMemoryPreference.setVisible(false);
            mInstantAppDomainsPreference.setVisible(false);
            mLaunchPreference.setVisible(false);
        }
        /// TCT}
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode) {
            case REQUEST_UNINSTALL:
                ...
                /// Added by yunqi.song.hz for JoyUI dualApp TASK 6766810
                //Begin deleted by xuhui.li.hz for XR 7351181,7350938 2019/01/24
                //if (mUserId == PackageCloneManager.getInstance().getCloneUserId()) onPackageRemoved();
                //End deleted by xuhui.li.hz for XR 7351181,7350938 2019/01/24
    ...
    private void uninstallPkg(String packageName, boolean allUsers, boolean andDisable) {
        ...
        /// Added by yunqi.song.hz for JoyUI dualApp TASK 6766810,@TCT{
        if (mUserId == PackageCloneManager.getInstance().getCloneUserId()) {
            uninstallIntent.putExtra(Intent.EXTRA_USER, new UserHandle(mUserId));
        }
        /// TCT}
    ...
    private void forceStopPackage(String pkgName) {
        ...
        if (mUserId != PackageCloneManager.getInstance().getCloneUserId()) {
            ...
            am.forceStopPackageAsUser(pkgName, mUserId);
        }
        /// TCT}
    ...
    void checkForceStop() {
        if (mDpm.packageHasActiveAdmins(mPackageInfo.packageName)) {
            ...
            Log.d(LOG_TAG, "Sending broadcast to query restart status for "
                    + mAppEntry.info.packageName + ", EXTRA_UID:" + mAppEntry.info.uid + ", EXTRA_USER_HANDLE:" + UserHandle.getUserId(mAppEntry.info.uid));
            getActivity().sendOrderedBroadcastAsUser(intent, (mUserId != -1) ? new UserHandle(mUserId) : UserHandle.CURRENT, null,
