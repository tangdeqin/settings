...
public class ManageApplications extends InstrumentedPreferenceFragment
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (mListContainer != null) {
            ...
            //Begin modify by liu.zheng.hz for XR6873798 un-use frameworks resource on 2018/08/27
            int emptyId = getActivity().getResources().getIdentifier("empty","id","android");
            View emptyView = mListContainer.findViewById(emptyId);
            //End modify by liu.zheng.hz for XR6873798 un-use frameworks resource on 2018/08/27
    ...
    void createHeader() {
        ...
        if (mListType == LIST_TYPE_MAIN) {
            if (UserManager.get(getActivity()).getUserProfiles().size() > 1) {
                ...
                for(UserHandle userHandle : UserManager.get(getActivity()).getUserProfiles()) {
                    if(userHandle.getIdentifier() == PackageCloneManager.getInstance().getCloneUserId()) {
                        if(UserManager.get(getActivity()).getUserProfiles().size() > 2) {
                            mFilterAdapter.disableFilter(FILTER_APPS_PERSONAL);
                            mFilterAdapter.disableFilter(FILTER_APPS_WORK);
                        }
                        break;
                    }
                }
    ...
    static class ApplicationsAdapter extends BaseAdapter implements Filterable,
        ...
        //BEGIN-added by ruichang.liang for task6699354 on 2018-8-8
        private final String ORANGE_APPS = "com.orange.aura.oobe;com.orange.update;com.orange.clock;com.orange.widgets.mostusedapps;com.orange.widget.tips";
        //END-added by ruichang.liang for task6699354 on 2018-8-8
        ...
        public void onRebuildComplete(ArrayList<AppEntry> entries) {
            ...
            //BEGIN-added by ruichang.liang for task6699354 on 2018-8-8
            if(mEntries != null){
                Iterator<AppEntry> iter = mEntries.iterator();
                ApplicationsState.AppEntry item = null;
                int isEnablePOMResId = mContext.getResources().getIdentifier("def_framework_enable_orange_POM", "bool", "com.tct");
                boolean isEnablePOM = isEnablePOMResId != 0 && mContext.getResources().getBoolean(isEnablePOMResId);
                while(iter.hasNext()){
                    item = iter.next();
                    if(item.info != null
                            && item.info.packageName != null
                            && ORANGE_APPS.contains(item.info.packageName)
                            && !item.info.enabled
                            && isEnablePOM ){
                        Log.d(TAG, "removing "+ item.info.packageName);
                        iter.remove();
                        continue;
                    }
                }
