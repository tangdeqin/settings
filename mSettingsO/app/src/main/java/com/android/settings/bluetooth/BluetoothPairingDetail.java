...
public class BluetoothPairingDetail extends DeviceListPreferenceFragment implements
    ...
    public void onDeviceBondStateChanged(CachedBluetoothDevice cachedDevice, int bondState) {
        if (bondState == BluetoothDevice.BOND_BONDED) {
            ...
            //[TCT-ROM][Solution] Begin added by chonglin.pan for XR6622418 on 2018/07/20
            if (Utils.getBluetoothPairingDialogState() && cachedDevice.getDevice() != null
                    && cachedDevice.getDevice().getBluetoothClass().getDeviceClass()
                    == BluetoothClass.Device.PERIPHERAL_KEYBOARD ) {
                Log.d(TAG, "Utils.getBluetoothPairingDialogState()");
                try {
                    Utils.getBluetoothPairingDialog().dismiss();
                    Thread.sleep(500);
                } catch (Exception e) {
                    Log.d(TAG, "e = " + e);
                }
