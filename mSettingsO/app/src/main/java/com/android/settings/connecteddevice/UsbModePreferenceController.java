...
import android.util.Log;//Added by miaoliu for XRP24322 on 2018/9/26
public class UsbModePreferenceController extends AbstractPreferenceController
    ...
    private void updataSummary(Preference preference, int mode) {
        if (preference != null) {
            Log.d("UsbModePreferenceController","updateSummary: isConnected = "+mUsbReceiver.isConnected());//Added by miaoliu for XRP24322 on 2018/9/26
            if (mUsbReceiver.isConnected()) {
                preference.setEnabled(true);
                preference.setSummary(getSummary(mode));
            } else {
                preference.setSummary(R.string.disconnected);
                Log.d("UsbModePreferenceController","updateSummary: setEnable before ");//Added by miaoliu for XRP24322 on 2018/9/26
                preference.setEnabled(false);
                Log.d("UsbModePreferenceController","updateSummary: setEnable after "+preference.isEnabled());//Added by miaoliu for XRP24322 on 2018/9/26
            }
        }
    }
