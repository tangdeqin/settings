...
public class BillingCycleSettings extends DataUsageBase implements
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (mEnableDataLimit == preference) {
            ...
            boolean enabled = (Boolean) newValue;
            if (enabled) {
                //Begin modified by miaoliu for XR7102824 on 2018/11/7
                //setPolicyWarningBytes(mDataUsageController.getDefaultWarningLevel());
                long warningBytes = mDataUsageController.getDefaultWarningLevel();
                warningBytes = getCustomDataWarningBytes(getContext(),warningBytes);
                setPolicyWarningBytes(warningBytes);
                //End modified by miaoliu for XR7102824 on 2018/11/7
            } else {
    ...
    public static class ConfirmLimitFragment extends InstrumentedDialogFragment implements
        ...
        public static void show(BillingCycleSettings parent) {
            ...
            //Begin modified by miaoliu for XR
            //limitBytes = Math.max(5 * GB_IN_BYTES, minLimitBytes);
             limitBytes = getCustomDataLimitBytes(parent.getContext(), minLimitBytes);
            //End modified by miaoliu for XR
            final Bundle args = new Bundle();
     ...
     //Begin added by miaoliu for XR7102824 on 2018/11/7
     private long getCustomDataWarningBytes(Context context, long originValue){
        //Begin modified by miaoliu for XR7107006 on 2018/11/15
        int ResourceId = context.getResources().getIdentifier("def_networks_datausage_warningthreshold", "integer", "com.tct");
       //End modified by miaoliu for XR7107006 on 2018/11/15
        long customWarning = ResourceId > 0 ? MB_IN_BYTES * context.getResources().getInteger(ResourceId) : 0;
        return customWarning > 0 ? customWarning : originValue;
    }

    private static long getCustomDataLimitBytes(Context context, long minLimitBytes) {
        //Begin modified by miaoliu for XR7107006 on 2018/11/15
        int id = context.getResources().getIdentifier("def_networks_datausage_forcestopthreshold", "integer", "com.tct");
        long customLimit = id > 0 ? context.getResources().getInteger(id) : 0;
        //End modified by miaoliu for XR7107006 on 2018/11/15
        return customLimit > 0 ?
                Math.max(MB_IN_BYTES * customLimit, minLimitBytes) :
                Math.max(5 * GB_IN_BYTES, minLimitBytes);
    }
    //End added by miaoliu for XR7102824 on 2018/11/7
     //End modified by miaoliu for XR7107006 on 2018/11/15
}
