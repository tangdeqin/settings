...
public class CellDataPreference extends CustomDialogPreference implements TemplatePreference {
    ...
    protected void onClick(DialogInterface dialog, int which) {
        ...
        if (mMultiSimDialog) {
            ...
            //End added by ruihua.zhang.hz for tclPlugin telecomcode on 2018/08/21
            //Begin modified by miaoliu for XR7143505 on 2018/12/10
            //mSubscriptionManager.setDefaultDataSubId(mSubId);
             if(TclInterfaceAdapter.hasTclPlugin(getContext())) {
                  TclInterfaceAdapter.setDefaultDataSubId(getContext(), mSubId);
              }else {
                   mSubscriptionManager.setDefaultDataSubId(mSubId);
             }
            //End modified by miaoliu for XR7143505 on 2018/12/10
            setMobileDataEnabled(true);
            disableDataForOtherSubscriptions(mSubId);
        } else {
    ...
    private boolean getDataRoaming() {
        ...
        try {
            if (mTelephonyManager.getSimCount() == 1) {
                ...
                result =  TelephonyManager.getIntWithSubId(resolver, Settings.Global.DATA_ROAMING, mSubId) != 0;
            }
        } catch (Settings.SettingNotFoundException snfe) {
            Log.d(TAG, "isDomesticRoamingEnabled false");
            return false;
        }
        return result;
    }
                    ...
                    public void onClick(DialogInterface dialog, int id) {
                        ...
                        if (mTelephonyManager.getSimCount() > 1 && (defaultDataSubId != subId)) {
                            //Begin modified by miaoliu for XR7143505 on 2018/12/10
                             ...
                             if(TclInterfaceAdapter.hasTclPlugin(getContext())) {
                                  TclInterfaceAdapter.setDefaultDataSubId(getContext(), subId);
                              }else {
                                   subscriptionManager.setDefaultDataSubId(subId);
                             }
                            //End modified by miaoliu for XR7143505 on 2018/12/10
   ...
   private void updateScreenEnableState() {
        //Begin modified by miaoliu for XR7137438 on 2018/11/21
        //setEnabled(!mIsAirplaneModeOn );
         boolean isCapabilitySwitching = TclInterfaceAdapter.isCapabilitySwitching(getContext());
        setEnabled(!mIsAirplaneModeOn && !isCapabilitySwitching
                && TclInterfaceAdapter.isRadioOn(mSubId, getContext()));
        //End modified by miaoliu for XR7137438 on 2018/11/21
   }
