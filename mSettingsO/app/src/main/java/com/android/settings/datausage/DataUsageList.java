...
public class DataUsageList extends DataUsageBase {
    ...
    public void bindStats(NetworkStats stats, int[] restrictedUids) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (UserHandle.isApp(uid)) {
                if (profiles.contains(new UserHandle(userId))) {
                    if (userId != currentUserId
                            ...
                            && userId != PackageCloneManager.getInstance().getCloneUserId()) {
            ...
            boolean isReservedPkg = false;
            String[] pkgNames = pm.getPackagesForUid(collapseKey);
            if(pkgNames != null) {
                for(String pkgName : pkgNames){
                    if(PackageCloneManager.isReservedClonedPkg(pkgName)
                            && userId == PackageCloneManager.getInstance().getCloneUserId()){
                        isReservedPkg = true;
                        break;
                    }
