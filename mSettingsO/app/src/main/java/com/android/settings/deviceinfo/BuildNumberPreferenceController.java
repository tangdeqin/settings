...
//begin zhixiong.liu.hz for 6104240 2018/7/18
import android.telephony.TelephonyManager;
import android.os.SystemProperties;  
//end zhixiong.liu.hz for 6104240 2018/7/18
        ...
        if (preference != null) {
            try {
                 //begin zhixiong.liu.hz for 6104240 2018/7/18
                  /*
                   * displayType
                   * 0x00: Show build number as ro.build.display.id
                   * 0x01: Show build number as svn
                   * 0x02: Show build number as IMEI sv
                   * 0x03: Show build number as version+perso //Added by zubai.li for XR6230340 on 2018.04.24
                  */                
                 String svn = SystemProperties.get("ro.def.software.svn");
                 String sys_ver = SystemProperties.get("ro.rom.yinfo");//zhixiong.liu.hz for rom version info P24071 20181008
                 String buildVersion;
                 int displayType = mContext.getResources().getInteger(R.integer.def_settings_display_build_number);

                 if(!TextUtils.isEmpty(svn) && displayType == 0x01) {
                     String countryCode = SystemProperties.get("ro.def.svn.countrycode");
                     buildVersion= svn;
                     if(null!=countryCode) {
                         buildVersion = svn + countryCode;
                     }
                     preference.setSummary(BidiFormatter.getInstance().unicodeWrap(buildVersion));
                 }
                 //begin zhixiong.liu.hz for rom version info P24071 20181008
                 else if(displayType == 0x02){
                     TelephonyManager telephonyManager = TelephonyManager.from(mContext);
                     buildVersion = telephonyManager.getDeviceSoftwareVersion();
                     preference.setSummary(BidiFormatter.getInstance().unicodeWrap(buildVersion));
                     
                 }
                 else if(!TextUtils.isEmpty(sys_ver) && displayType == 0x03){  
                     buildVersion = sys_ver.substring(1, 4) + sys_ver.substring(6, 7) + "+"
                                     + sys_ver.substring(4, 6) + sys_ver.substring(7, 8);
                     preference.setSummary(BidiFormatter.getInstance().unicodeWrap(buildVersion));
                 }

                 //end zhixiong.liu.hz for rom version info P24071 20181008
                 else{
