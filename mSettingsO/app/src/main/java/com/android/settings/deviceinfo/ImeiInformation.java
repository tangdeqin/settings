...
//begin zhixiong.liu.hz for P23576 20180905
...
public class ImeiInformation extends SettingsPreferenceFragment {
        ...
        if (phone != null) {
            if (phone.getPhoneType() == TelephonyManager.PHONE_TYPE_CDMA) {
                ...
                if (phone.getLteOnCdmaMode() == PhoneConstants.LTE_ON_CDMA_TRUE) {
                    ...
                    //begin zhixiong.liu.hz for XR6620011  20180719
                    //setSummaryTextAsDigit(KEY_IMEI_SV, phone.getDeviceSvn());
                    setSummaryTextAsDigit(KEY_IMEI_SV, getcustomIMEISV(phone.getImei(), phone.getDeviceSvn()));
                    //end zhixiong.liu.hz for XR6620011  20180719
                } else {
    ...
    //begin zhixiong.liu.hz for XR6620011  20180719
    private String getcustomIMEISV(String imei, String deviceSvn){
        String imei_sv = deviceSvn;
         //begin zhixiong.liu.hz for P23576 20180905
        String svn = SystemProperties.get("ro.def.software.svn");
        if ((!TextUtils.isEmpty(svn) && svn.length() >= 5) && !TextUtils.isEmpty(imei)) {
            if(!getResources().getBoolean(R.bool.def_Settings_imeisv_display_style)){
                imei_sv = imei.substring(0, imei.length() - 1) + svn.substring(svn.length() - 2);
            }
            else{
                imei_sv = svn.substring(svn.length() - 2);
            }
        }   
        //if (!TextUtils.isEmpty(imei) && !getResources().getBoolean(R.bool.def_Settings_imeisv_display_style)) {
        //   imei_sv = imei.substring(0, imei.length() - 1) + deviceSvn;
        //}
        //end zhixiong.liu.hz for P23576 20180905
        return imei_sv;
    }
    //end zhixiong.liu.hz for XR6620011  20180719
