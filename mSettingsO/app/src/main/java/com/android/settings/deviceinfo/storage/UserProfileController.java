...
public class UserProfileController extends AbstractPreferenceController implements
    ...
    public void displayPreference(PreferenceScreen screen) {
        ...
        if(mUser.id == PackageCloneManager.getInstance().getCloneUserId()) {
            mStoragePreference.setTitle(mContext.getString(com.android.settings.R.string.category_clone));
        }
        /// TCT}
    ...
    public boolean handlePreferenceTreeClick(Preference preference) {
        if (preference != null && mStoragePreference == preference) {
            ...
            /// Added by yunqi.song.hz for JoyUI dualApp TASK 6766810,@TCT{
            String title = mUser.name;
            if(mUser.id == PackageCloneManager.getInstance().getCloneUserId()) {
                title = mContext.getString(com.android.settings.R.string.category_clone);
            }
            /// TCT}
            /// Modified by yunqi.song.hz for JoyUI dualApp TASK 6766810
