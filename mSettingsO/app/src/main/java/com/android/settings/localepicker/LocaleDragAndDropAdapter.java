...
class LocaleDragAndDropAdapter
    ...
    void addLocale(LocaleStore.LocaleInfo li) {
        // Begin added by xi.peng for P23984 on 2018/09/22
        final Locale locale = li.getLocale();
        for (LocaleStore.LocaleInfo feedItemListItem : mFeedItemList) {
            if (feedItemListItem.getLocale().equals(locale)) {
                Log.e(TAG, "Language has already exsit: "
                                   + locale.getCountry() + "-" + locale.getLanguage());
                return;
            }
        }
        // End added by xi.peng for P23984 on 2018/09/22
