...
public class LocaleListEditor extends SettingsPreferenceFragment
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // Begin Added by pengxi for P24117 on 2018/10/11
        if ("true".equals(android.os.SystemProperties.get("def.settings.locale.summary", "false"))) {
            int esInList = 0;
            for (LocaleStore.LocaleInfo li : feedsList) {
                if("es".equals(li.getLocale().getLanguage())) {
                    esInList = 1;
                    break;
                }
            }
            android.provider.Settings.Global.putInt(getContentResolver(), "es_in_language_list", esInList);
        }
        // End Added by pengxi for P24117 on 2018/10/11
