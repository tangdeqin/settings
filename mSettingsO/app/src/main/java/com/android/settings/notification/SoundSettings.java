...
//Begin add by miaoliu for XR5799410 on 2017/12/26
import android.content.IntentFilter;
import android.content.BroadcastReceiver;
import android.os.PowerManager;
//End add by miaoliu for XR5799410 on 2017/12/26
public class SoundSettings extends DashboardFragment {
    ...
    private BroadcastReceiver mPowerSaverReceiver = new BroadcastReceiver() {
        @Override
    ...
    /**
     * update the verUI when powersavemode change
     */
    private void updateVibtareUI() {
        Preference Ver = mProgressiveDisclosureMixin.findPreference(getPreferenceScreen(),"vibrate_on_touch");
         if(Ver != null){
            Ver.setEnabled(!mPowerManager.isPowerSaveMode());
        }
    }
    //End add by miaoliu for XR5799410 on 2017/12/26
    @Override
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mContext.registerReceiver(mPowerSaverReceiver, new IntentFilter(PowerManager.ACTION_POWER_SAVE_MODE_CHANGING));
        updateVibtareUI();
        //End add by miaoliu for XR5799410 on 2017/12/26
    }
//Begin add by miaoliu for XR5799410 on 2017/12/26
    ...
    public void onDestroy() {
        ...
        super.onDestroy();
        mContext.unregisterReceiver(mPowerSaverReceiver);
    }
    //End add by miaoliu for XR5799410 on 2017/12/26
    ...
    private static List<AbstractPreferenceController> buildPreferenceControllers(Context context,
        ...
        controllers.add(new HeadsetModePreferenceController(context,lifecycle));//Added by nanbing.zou for Task:6628160 at 2018-07-27

        controllers.add(new SilentModePreferenceController(context, lifecycle));//Add for task 6631231 by junliang.liu 2018.8.7

        controllers.add(new DualSoundSettingsController(context));//[TCT-ROM][SoundDual] Added by nanbing.zou for P23287 on 2018-08-31
        ...
        controllers.add(new VibrationSettingsPreferenceController(context));//Added by ronghui.yi for XR6628148 on 2018/08/09
        controllers.add(new EmergencyTonePreferenceController(context, fragment, lifecycle));

        controllers.add(new SoundEnhancementController(context));//[TCT-ROM][SoundEnhancement] Added by nanbing.zou for P23236 on 2018-09-05
        return controllers;
    }
