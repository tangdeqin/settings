...
import com.android.settings.privatemode.ChoosePrivacyModeLockPattern;
...
public class ChooseLockPattern extends SettingsActivity {
    ...
    public static class ChooseLockPatternFragment extends InstrumentedPreferenceFragment
        ...
        //Begin added by dongchi.chen for XR6700377 on 2018/08/02
        private static final int SET_PRIVACY_PASSWORD_REQUEST = 56;
                ...
                public void onPatternDetected(List<LockPatternView.Cell> pattern) {
                    if (mUiStage == Stage.NeedToConfirm || mUiStage == Stage.ConfirmWrong) {
                        ...
                        if (mChosenPattern.equals(pattern)) {
                            ...
                            || mUiStage == Stage.ChoicePrivacyNormalPasswordSame){//modified by dongchi.chen for XR6700377 on 2018/08/02
                        if (pattern.size() < LockPatternUtils.MIN_LOCK_PATTERN_SIZE) {
                            updateStage(Stage.ChoiceTooShort);
                        } else {
                            //Begin modified by dongchi.chen for XR6700377 on 2018/08/02
                            //mChosenPattern = new ArrayList<LockPatternView.Cell>(pattern);
                            //updateStage(Stage.FirstChoiceValid);
                            mChosenPattern = new ArrayList<LockPatternView.Cell>(pattern);
                            String p = LockPatternUtils.patternToString(mChosenPattern);
                            if (p.equals(mPrivacyModePassword) &&
                                    !mPrivacyModeHelper.isChangedTheUnlockScreenType(PASSWORD_QUALITY_SOMETHING)) {
                                updateStage(Stage.ChoicePrivacyNormalPasswordSame);
                            }else {
                                updateStage(Stage.FirstChoiceValid);
                            }
                            //end modified by dongchi.chen for XR6700377 on 2018/08/02
                        }
                    } else {
        ...
        protected enum Stage {
            ...
            ChoiceTooShort(
                    ...
                    ID_EMPTY_MESSAGE, true),
            //Begin added by dongchi.chen for XR6700377 on 2018/08/02
            ChoicePrivacyNormalPasswordSame(
                    R.string.lockpassword_confirm_admin_match_privacy_mode,
                    LeftButtonMode.Retry, RightButtonMode.ContinueDisabled,
                    ID_EMPTY_MESSAGE, true),
            //End added by dongchi.chen for XR6700377 on 2018/08/02
            FirstChoiceValid(
        ...
        private void startSaveAndFinish() {
            ...
            //Begin added by dongchi.chen for XR6700377 on 2018/08/02
            mPrivacyModeHelper.saveNormalPassword(LockPatternUtils.patternToString(mChosenPattern));
            //End added by dongchi.chen for XR6700377 on 2018/08/02
        }
        ...
        private void setupPrivacyModePassword() {
            ...
            boolean required = getActivity().getIntent().getBooleanExtra(
                    EncryptionInterstitial.EXTRA_REQUIRE_PASSWORD, true);
            //is diff for android 7.0
            intent = ChoosePrivacyModeLockPattern.createIntent(getActivity(), required, mChallenge, mUserId);
            intent = ChoosePrivacyModeLockPattern.createIntent(getActivity(), required, mChallenge, UserHandle.myUserId());
            intent.putExtra(ChooseLockSettingsHelper.EXTRA_KEY_NORMAL_PASSWORD, LockPatternUtils.patternToString(mChosenPattern));
            intent.putExtra(ChooseLockSettingsHelper.EXTRA_KEY_CURRENT_NORMAL_PASSWORD, mCurrentPattern);
