...
//Begin added by miaoliu for XRP10026452 on 2018/11/20  
import android.text.Selection;
import android.text.Spannable;
//End added by miaoliu for XRP10026452 on 2018/11/20
/**
...
class ConfigDialog extends AlertDialog implements TextWatcher,
    ...
    protected void onCreate(Bundle savedState) {
        ...
        if (mProfile.saveLogin) {
            ...
            Selection.setSelection((Spannable) mUsername.getText(),mProfile.username.length());//Added by miaoliu for XRP10026452 on 2018/11/20  
            Selection.setSelection((Spannable) mPassword.getText(),mProfile.password.length());//Added by miaoliu for XRP10027524 on 2018/12/10  
        }
        ...
        if (mEditing) {
            ...
            Selection.setSelection((Spannable) mName.getText(),mProfile.name.length());//Added by miaoliu for XRP10026452 on 2018/11/20
            Selection.setSelection((Spannable) mServer.getText(),mProfile.server.length());//Added by miaoliu for XRP10027524 on 2018/12/10  
        } else {
    ...
    private void showAdvancedOptions() {
        //Begin modified by miaoliu for XR7139070 on 2018/11/21
        ...
        //End modified by miaoliu for XR7139070 on 2018/11/21
    }
    ...
    private boolean validateAddresses(String addresses, boolean cidr) {
        try {
            for (String address : addresses.split(" ")) {
                ...
                byte[] bytes = myparseNumericAddress(address).getAddress();
    ...
    public static InetAddress myparseNumericAddress(String numericAddress) {
        ...
        try {
            addresses = Libcore.os.android_getaddrinfo(numericAddress, hints, NETID_UNSET);
        } catch (Exception ignored) {
        }
        InetAddress result = (addresses != null) ? addresses[0] : null;

        if (!(result instanceof Inet4Address) || numericAddress.indexOf(':') != -1) {

        }
        Libcore.os.inet_pton(2, numericAddress);
