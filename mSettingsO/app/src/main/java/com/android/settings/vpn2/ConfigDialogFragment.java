    ...
    public void onResume() {
        ...
        if (!KeyStore.getInstance().isUnlocked()) {
            if (!mUnlocking) {
                ...
                //Begin modified by miaoliu for XRP10024265 on 2018/10/10
                if(!getResources().getBoolean(R.bool.def_settings_not_necessary_pin_for_vpn)){
                    Credentials.getInstance().unlock(mContext);
                }
                //End modified by miaoliu for XRP10024265 on 2018/10/10
            } else {
