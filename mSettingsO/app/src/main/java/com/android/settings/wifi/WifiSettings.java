...
//Begin added by jiabin.zheng.hz for XR6687396 on 2018/07/28
...
//End added by jiabin.zheng.hz for XR6687396 on 2018/07/28
//Begin added by jiabin.zheng.hz for XR7137336 on 2018/11/21
import android.content.res.Configuration;
//End added by jiabin.zheng.hz for XR7137336 on 2018/11/21
...
public class WifiSettings extends RestrictedSettingsFragment
    ...
    //Begin added by jiabin.zheng.hz for XR6698339 on 2018/08/02
    private static final int MENU_ID_AUTO_JOIN = Menu.FIRST + 15;
    //End added by jiabin.zheng.hz for XR6698339 on 2018/08/02
    ...
    private final BroadcastReceiver mTclReceiver;
    ...
    //Begin added by jiabin.zheng.hz for XR7233263 on 2018/12/24
    //to mark connected ap preference whether config was null
    boolean mConnectedApConfigWasNull = false;

    long mLastUpdateTimestamp = 0;
    //Begin added by jiabin.zheng.hz for XR7233263 on 2018/12/24
    ...
    public WifiSettings() {
        ...
        mTclReceiver = new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, Intent intent) {
                handleEventTcl(intent);
            }
        };
        //End added by jiabin.zheng.hz for XR6687396 on 2018/07/28
    }
    ...
    //Begin added by jiabin.zheng.hz for XR6687396 on 2018/07/28
    private void handleEventTcl(Intent intent) {
        String action = intent.getAction();
        Log.d(TAG, "handleEvent(), action = " + action);
        if (WifiManager.WIFI_STATE_CHANGED_ACTION.equals(action)) {
            int state = intent.getIntExtra(WifiManager.EXTRA_WIFI_STATE, WifiManager.WIFI_STATE_UNKNOWN);
            if ((state == WifiManager.WIFI_STATE_ENABLED) && (mTctAploader != null)) {
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        //Begin modified by jiabin.zheng.hz for XR7233263 on 2018/12/24
        mWifiTracker = WifiTrackerFactory.create(
                getActivity(), this, mBgThread.getLooper(), false, true, false);
        //End modified by jiabin.zheng.hz for XR7233263 on 2018/12/24
        mWifiManager = mWifiTracker.getManager();

        //Begin added by jiabin.zheng.hz for XR6687768 on 2018/07/28
        mTctWifiMgr = TctWifiSettingsManager.getTctWifiSettingsManager(getActivity());
        mTctAploader = TctPredefineApLoader.getTctPredefineApLoader(getActivity());
        //End added by jiabin.zheng.hz for XR6687768 on 2018/07/28
    ...
    public void onResume() {
        ...
        //Begin added by jiabin.zheng.hz for XR6687396 on 2018/07/28
        activity.registerReceiver(mTclReceiver, mFilter);
        //End added by jiabin.zheng.hz for XR6687396 on 2018/07/28
    }
    ...
    public void onPause() {
        ...
        //Begin added by jiabin.zheng.hz for XR6687396 on 2018/07/28
        getActivity().unregisterReceiver(mTclReceiver);
        //End added by jiabin.zheng.hz for XR6687396 on 2018/07/28
    }
    ...
    public void onCreateContextMenu(ContextMenu menu, View view, ContextMenuInfo info) {
            ...
            if (preference instanceof LongPressAccessPointPreference) {
                ...
                //Begin added by jiabin.zheng.hz for XR6698339 on 2018/08/02
                WifiConfiguration cfg = mSelectedAccessPoint.getConfig();
                if (mTctWifiMgr.isVFProduct() && mTctWifiMgr.isPresetAP(cfg)) {
                    MenuItem item = menu.add(Menu.NONE, MENU_ID_AUTO_JOIN, 0, R.string.wifi_auto_join);
                    item.setCheckable(true);

                    boolean isAutoJoin = false;
                    if (mSelectedAccessPoint.isSaved()) {
                        if (cfg != null && cfg.getNetworkSelectionStatus() != null) {
                            isAutoJoin = !cfg.getNetworkSelectionStatus().isNetworkPermanentlyDisabled();
                        }
                    }

                    item.setChecked(isAutoJoin);
                    return;
                }
                //End added by jiabin.zheng.hz for XR6698339 on 2018/08/02
                ...
                if (mSelectedAccessPoint.isSaved() || mSelectedAccessPoint.isEphemeral()) {
                    ...
                    //Begin modified by jiabin.zheng.hz for XR6698339 on 2018/08/02
                    //old: menu.add(Menu.NONE, MENU_ID_FORGET, 0, R.string.wifi_menu_forget);
                    if (mTctWifiMgr.isPresetAP(mSelectedAccessPoint.getConfig())
                            && mTctWifiMgr.isNoforget()) {
                        menu.add(Menu.NONE, MENU_ID_FORGET, 0, R.string.bluetooth_device_context_disconnect);
                    } else {
                        menu.add(Menu.NONE, MENU_ID_FORGET, 0, R.string.wifi_menu_forget);
                    }
                    //End modified by jiabin.zheng.hz for XR6698339 on 2018/08/02
                }
                if (mSelectedAccessPoint.isSaved()) {
                    //Begin modified by jiabin.zheng.hz for XR6698339 on 2018/08/02
                    //menu.add(Menu.NONE, MENU_ID_MODIFY, 0, R.string.wifi_menu_modify);
                    if (!mTctWifiMgr.isPresetAP(mSelectedAccessPoint.getConfig())
                            || !mTctWifiMgr.isNoModify()) {
                        menu.add(Menu.NONE, MENU_ID_MODIFY, 0, R.string.wifi_menu_modify);
                    }
                    //End modified by jiabin.zheng.hz for XR6698339 on 2018/08/02

                    NfcAdapter nfcAdapter = NfcAdapter.getDefaultAdapter(getActivity());
                    //Begin modified by taoyang.hz for XR7099284 on 2018/11/20
                    if (nfcAdapter != null && nfcAdapter.isEnabled() &&
                            mSelectedAccessPoint.getSecurity() == AccessPoint.SECURITY_PSK) {
                        // Only allow writing of NFC tags for password-protected networks.
                        menu.add(Menu.NONE, MENU_ID_WRITE_NFC, 0, R.string.wifi_menu_write_to_nfc);
                    }
                    //End modified by taoyang.hz for XR7099284 on 2018/11/20
                }
            }
    }
    ...
    public boolean onContextItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case MENU_ID_FORGET: {
                //Begin added by jiabin.zheng.hz for XR6698339 on 2018/08/02
                if (mTctWifiMgr.isNoforget() && mTctWifiMgr.isPresetAP(mSelectedAccessPoint.getConfig())) {
            ...
            case MENU_ID_AUTO_JOIN:
                ...
                //it is the original state of item
                if (item.isChecked()) {
                    mWifiManager.disableNetwork(mSelectedAccessPoint.getConfig().networkId);
                } else {
    ...
    private void updateAccessPointsDelayed() {
        ...
        if (getActivity() != null && !mIsRestricted && mWifiManager.isWifiEnabled()) {
            setProgressBarVisible(true);
            //Begin added by jiabin.zheng.hz for XR7233263 on 2018/12/24
            //make the update interval no less than updateInterval
            final long updateInterval = 400;
            long systime = System.currentTimeMillis();
            if (systime < mLastUpdateTimestamp) {
                //means more than one update runnable in queue
                return;
            }

            long interval = systime - mLastUpdateTimestamp;
            if (interval < updateInterval) {
                //wait for last update runnable execute
                getView().postDelayed(mUpdateAccessPointsRunnable, 300 + (updateInterval-interval) /* delay milliseconds */);
                //store next update timestamp
                mLastUpdateTimestamp = systime + (updateInterval-interval);
                return;
            }
            mLastUpdateTimestamp = systime;
            //End added by jiabin.zheng.hz for XR7233263 on 2018/12/24
    ...
    private boolean configureConnectedAccessPointPreferenceCategory(
        ...
        if (mConnectedAccessPointPreferenceCategory.getPreferenceCount() == 0) {
            //Begin added by jiabin.zheng.hz for XR7233263 on 2018/12/24
            if (connectedAp.getConfig() == null) {
                mConnectedApConfigWasNull = true;
                Log.d(TAG, "added ConnectedAccessPoint, config is null");
            }
            //End added by jiabin.zheng.hz for XR7233263 on 2018/12/24
        ...
        //Begin added by jiabin.zheng.hz for XR7233263 on 2018/12/24
        if (mConnectedApConfigWasNull) {
            if (connectedAp.getConfig() == null) {
                Log.d(TAG, "Connected AccessPoint config still null");
            } else {
                Log.d(TAG, "Connected AccessPoint config Was null, need reset preference");
                mConnectedApConfigWasNull = false;
                removeConnectedAccessPointPreference();
                addConnectedAccessPointPreference(connectedAp);
                return true;
            }
    ...
    private void setAdditionalSettingsSummaries() {
        ...
        //Begin modified by jiabin.zheng.hz for XR6873798 on 2018/08/28
        // final int defaultWakeupAvailable = getResources().getInteger(
        //         com.android.internal.R.integer.config_wifi_wakeup_available);
        int resId = getResources().getIdentifier("config_wifi_wakeup_available","integer","android");
        final int defaultWakeupAvailable = getResources().getInteger(resId);
        //End modified by jiabin.zheng.hz for XR6873798 on 2018/08/28
    ...
    /* package */ void submit(WifiConfigController configController) {
        ...
        if (TctWifiUtil.isSimMethodAP(config)) {
            if (mTctWifiMgr.getSimSlot(config) == -1 && mTctWifiMgr.isDualSim()) {
