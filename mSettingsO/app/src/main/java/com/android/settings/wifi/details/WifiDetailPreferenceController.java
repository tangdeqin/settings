...
public class WifiDetailPreferenceController extends AbstractPreferenceController
    ...
    public void displayPreference(PreferenceScreen screen) {
       ...
       //Begin added by jiabin.zheng.hz for XR6698339 on 2018/08/02
        if (mTctWifiMgr.isNoforget() && mTctWifiMgr.isPresetAP(mAccessPoint.getConfig())) {
            String strDisconnet = mContext.getResources().getString(R.string.bluetooth_device_context_disconnect);
            mForgetButton.setText(strDisconnet);
        }
       //End added by jiabin.zheng.hz for XR6698339 on 2018/08/02
    ...
    private boolean canForgetNetwork() {
        //Begin added by jiabin.zheng.hz for XR6698339 on 2018/08/02
        if (mTctWifiMgr.isPresetAP(mAccessPoint.getConfig()) && mTctWifiMgr.isVFProduct()) {
            return false;
        }
        //End added by jiabin.zheng.hz for XR6698339 on 2018/08/02
    ...
    private void forgetNetwork() {
        if (mWifiInfo != null && mWifiInfo.isEphemeral()) {
            ...
            if (mWifiConfig.isPasspoint()) {
               ...
               //old: mWifiManager.forget(mWifiConfig.networkId, null /* action listener */);
                if (mTctWifiMgr.isNoforget() && mTctWifiMgr.isPresetAP(mWifiConfig)) {
                    mWifiManager.disableNetwork(mWifiConfig.networkId);
                }else{
