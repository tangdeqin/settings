...
public class HelpUtils {
    ...
    public static boolean prepareHelpMenuItem(final Activity activity, MenuItem helpMenuItem,
        ...
        if (Global.getInt(activity.getContentResolver(), Global.DEVICE_PROVISIONED, 0) == 0) {
            //Begin added by miaoliu for XRP10024261 on 2018/10/17
             if (TextUtils.isEmpty(helpUriString)) {
                 // The help url string is empty or null, so set the help menu item to be invisible.
                helpMenuItem.setVisible(false);
              } 
            //End added by miaoliu for XRP10024261 on 2018/10/17
            return false;
        }
        if (TextUtils.isEmpty(helpUriString)) {
            // The help url string is empty or null, so set the help menu item to be invisible.
    ...
    public static void addIntentParameters(Context context, Intent intent, String backupContext,
        ...
        int sendPackageNameId = resources.getIdentifier("config_sendPackageName","bool","android");
        boolean includePackageName = resources.getBoolean(sendPackageNameId);

        if (sendPackageName && includePackageName) {
            String[] packageNameKey =
                    {resources.getString(resources.getIdentifier("config_helpPackageNameKey","string","android"))};
            String[] packageNameValue =
                    {resources.getString(resources.getIdentifier("config_helpPackageNameValue","string","android"))};
            String helpIntentExtraKey =
                    resources.getString(resources.getIdentifier("config_helpIntentExtraKey","string","android"));
            String helpIntentNameKey =
                    resources.getString(resources.getIdentifier("config_helpIntentNameKey","string","android"));
            String feedbackIntentExtraKey =
                    resources.getString(resources.getIdentifier("config_feedbackIntentExtraKey","string","android"));
            String feedbackIntentNameKey =
                    resources.getString(resources.getIdentifier("config_feedbackIntentNameKey","string","android"));
