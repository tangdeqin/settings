...
public class ApplicationsState {
    ...
    void doResumeIfNeededLocked() {
        ...
        for (UserInfo user : mUm.getProfiles(UserHandle.myUserId())) {
            ...
            if(user.id == PackageCloneManager.getInstance().getCloneUserId()) continue;
            try {
    ...
    public AppEntry getEntry(String packageName, int userId) {
        ...
        //Begin added by miaoliu for XRP10025443 on 2018/11/03
        if (mInterestingConfigChanges.applyNewConfig(mContext.getResources())) {
            // If an interesting part of the configuration has changed, we
            // should completely reload the app entries.
            clearEntries();
         }
        //End added by miaoliu for XRP10025443 on 2018/11/03
        synchronized (mEntriesMap) {
            /// Added by yunqi.song.hz for JoyUI dualApp TASK 6766810,@TCT{
            if (mEntriesMap.indexOfKey(userId) < 0) {
                mEntriesMap.put(userId, new HashMap<String, AppEntry>());
            }
            ///TCT}
