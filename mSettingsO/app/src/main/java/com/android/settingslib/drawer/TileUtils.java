...
import com.android.settings.Utils;//Added by miaoliu for XRP23480 on 2018/9/7
public class TileUtils {
    ...
    public static List<DashboardCategory> getCategories(Context context,
        ...
        Log.d(LOG_TAG, "getCategories setup -" + setup);
        for (UserHandle user : userManager.getUserProfiles()) {
            /// Added by yunqi.song.hz for JoyUI dualApp TASK 6766810
            if(user.getIdentifier() == PackageCloneManager.getInstance().getCloneUserId()) continue;
    ...
    public static void getTilesForIntent(
        ...
        for (ResolveInfo resolved : results) {
            ...
            //Begin added by miaoliu for XRP23480 on 2018/9/3
            // if (Utils.checkPackageExist(context, "com.tct.onetouchbooster")
            //         && "com.android.settings".equals(activityInfo.packageName)
            //         && null != activityInfo.name
            //         && (activityInfo.name.endsWith("StorageDashboardActivity")
            //         || activityInfo.name.endsWith("PowerUsageSummaryActivity")
            //         || activityInfo.name.endsWith("ConfigureNotificationSettingsActivity"))) {
                
            //     continue;
            // }
            //End added by miaoliu for XRP23480 on 2018/9/3
            //begin add by zhixiong.liu.hz  for 7445461 on 20190212 
            if(resolved.activityInfo.name.equals("com.tct.appclone.DualAppActivity")
                && !context.getResources().getBoolean(com.android.settings.R.bool.def_show_dual_app_ui)){
                continue;
            }
            //end add by zhixiong.liu.hz  for 7445461 on 20190212 
            // Load category
            ...
            if (tile == null) {
                //Begin added by miaoliu for XR7146491 on 2018/12/26
                if ("com.android.settings".equals(activityInfo.packageName)
                    && null != activityInfo.name
                    && (activityInfo.name.endsWith("StorageDashboardActivity")
                    || activityInfo.name.endsWith("PowerUsageSummaryActivity")
                    || activityInfo.name.endsWith("ConfigureNotificationSettingsActivity"))
                     && Utils.checkPackageExist(context, "com.tct.onetouchbooster")) {           
                    continue;
                }
                //End added by miaoliu for XR7146491 on 2018/12/26
                tile = new Tile();
                tile.intent = new Intent().setClassName(
                        activityInfo.packageName, activityInfo.name);
                tile.category = categoryKey;
                tile.priority = resolved.priority;//old:usePriority ? resolved.priority : 0;//Modified by miaoliu for XR on 2018/8/21
                tile.metaData = activityInfo.metaData;
                updateTileData(context, tile, activityInfo, activityInfo.applicationInfo,
                        pm, providerMap, forceTintExternalIcon);
                //Begin added by miaoliu for XR5354664 on 2017/9/21
                if(!usePriority){
                    if(tile.intent.getComponent().getClassName().equals("com.google.android.gms.app.settings.GoogleSettingsIALink")){
                        tile.icon = Icon.createWithResource(context, com.android.settings.R.drawable.ic_settings_google);
                        tile.isIconTintable = false;
                    }
                }
                //End added by miaoliu for XR5354664 on 2017/9/21
        ...
        if (applicationInfo.isSystemApp()) {
            ...
            try {
                ...
                if (forceTintExternalIcon
                    ...
                    isIconTintable = true;
                    //forceTintIcon = true;//Deleted by miaoliu for XR6107743 on 2018/3/29
                }
