...
public class WifiTracker {
    ...
    */
    ...
    //Begin added by jiabin.zheng.hz for XR7233263 on 2018/12/24
    long mLastAllUpdateTimestamp = 0;
    //Begin added by jiabin.zheng.hz for XR7233263 on 2018/12/24
            ...
            for (WifiConfiguration config : configs) {
                ...
                if (mIncludeSaved) {
                    ...
                    for (ScanResult result : results) {
                        //Begin modified by jiabin.zheng.hz for XR7233220 on 2018/12/21
                        //if (result.SSID.equals(accessPoint.getSsidStr())) {
                        if (accessPoint.matches(result)) {
                        //End modified by jiabin.zheng.hz for XR7233220 on 2018/12/21
                            apFound = true;
                            break;
                        }
                    }
    ...
    private void copyAndNotifyListeners(boolean notifyListeners) {
        ...
        //Begin added by jiabin.zheng.hz for XR7233263 on 2018/12/24
        //make all acceepoint update interval no less than 600ms
        boolean needUpdateAllAccesspoint = true;
        long systime = System.currentTimeMillis();
        if (systime - mLastAllUpdateTimestamp < 800) {
            needUpdateAllAccesspoint = false;
        } else {
            mLastAllUpdateTimestamp = systime;
        }
        //End added by jiabin.zheng.hz for XR7233263 on 2018/12/24
        ...
        if (notificationMap != null && notificationMap.size() > 0) {
            for (AccessPoint accessPoint : updatedAccessPoints) {
                ...
                //do not update too frequent if a network is not saved
                if ((notificationType & AccessPointListenerAdapter.AP_CHANGED) != 0
                        && (accessPoint.isSaved() || needUpdateAllAccesspoint)) {
